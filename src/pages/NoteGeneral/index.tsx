...
import {
  ...
  Note,
  ContainerList,
} from "./styles.NoteSelect";
import { apiNota, apiNotaRanking } from "../../services/notaGeneral";
import ButtonSearch from "../../components/ButtonSearch";

const NoteGeneral: React.FC = () => {
  const [visible, setVisible] = useState(false);
  const [loading, setLoading] = useState(false);
  const [vehiclesOrDrivers, setVehiclesOrDrivers] = useState<INotaGeneral>(
    {} as INotaGeneral
  );
  const [searchVehicleOrDriver, setSearchVehicleOrDrive] =
    useState<IVehicleOrDriver>({} as IVehicleOrDriver);
        ...
        <Center onClick={() => setVisible(false)}>
          <Box>
            <Row>
              ...
              <TitleNote>
                {" "}
                NOTA GENERAL DE LA FLOTA SELECCIONADA{" "}
              </TitleNote>{" "}
            </Row>
              ...
              {vehiclesOrDrivers.scores?.map((vehicleOrDrive, index) => {
                return (
                  <MiddleBoxNoteSelect
                    key={index}
                    isLightBlue={
                      vehicleOrDrive?.score_type === "conditions"
                        ? "lightblue"
                        : ""
                    }
                    isDarkBlue={
                      vehicleOrDrive?.score_type === "general" ? "darkblue" : ""
                    }
                  >
                    <h5>{vehicleOrDrive?.score_type}</h5>
                    <h4>
                      {" "}
                      {vehicleOrDrive?.last_period_grade
                        ? vehicleOrDrive?.last_period_grade
                        : "0.0"}{" "}
                    </h4>
                    <p>
                      {vehicleOrDrive.status === "equal" ? (
                        ...
                        <Icon>
                          {" "}
                          <i
                            className={`fas fa-sort-${vehicleOrDrive.status}
                      ${vehicleOrDrive.status === "up" ? "up" : "down"}`}
                          ></i>{" "}
            ...
            <Row>
              {" "}
              <TitleNote> RANKING DEL PERIODO </TitleNote>{" "}
            </Row>
            <Row>
              <LittleBox
            ...
            <ContainerList>
              <Note>
                ...
                <ul>
                  ...
                  {searchVehicleOrDriver.top_positions?.map(
                    (vehicleOrDriveTopPosition) => {
                      return (
                        <li key={vehicleOrDriveTopPosition?.id}>
                          ...
                          <strong>
                            {" "}
                            {
                              vehicleOrDriveTopPosition?.current_period_grade
                            }{" "}
                          </strong>
                          <p>
                            {vehicleOrDriveTopPosition?.current_period_grade}
                            <i
                              className={`fas fa-sort-${vehicleOrDriveTopPosition.status}`}
                            ></i>{" "}
                          </p>
                        </li>
                      );
